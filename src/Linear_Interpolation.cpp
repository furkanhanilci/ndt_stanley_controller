#include "../include/Linear_Interpolation.h"
...
Linear_Interpolation::Linear_Interpolation(const std::vector<double>& x_vals, const std::vector<double>& y_vals, double interp_distance_res) : INTERP_DISTANCE_RES(interp_distance_res)
{
    ...
    for (size_t i = 0; i < x_vals.size(); i++) {
        //std::cout<< "x_vals[i]: " << x_vals[i] << std::endl;
        //std::cout<< "y_vals[i]: " << y_vals[i] << std::endl;
        //Eigen::VectorXd waypoint = Eigen::VectorXd::Zero(2);
...
void Linear_Interpolation::interpolateWaypoints() {
    ...
    */
    ...
    // fh
    // Check if there are enough waypoints to perform interpolation.
    if (waypoints.size() < 1) {
        // Handle the case where there are not enough waypoints.
        // For example, you could simply return from the function or throw an exception.
        std::cerr << "Insufficient waypoints for interpolation (need at least 2)." << std::endl;
        return; // Or throw std::runtime_error("Insufficient waypoints for interpolation.");
    }
    ...
    wp_distance.push_back(0.0); // Keep this line if you need a terminal value in wp_distance.
